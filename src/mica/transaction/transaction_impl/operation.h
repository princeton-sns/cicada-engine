...
template <class StaticConfig>
template <class DataCopier>
bool Transaction<StaticConfig>::upsert_row(RAH& rah, Table<StaticConfig>* tbl,
                                           uint16_t cf_id, uint64_t row_id,
                                           bool check_dup_access,
                                           uint64_t data_size,
                                           const DataCopier& data_copier) {
...
bool Transaction<StaticConfig>::peek_row_replica(RAH& rah, Table<StaticConfig>* tbl,
  ...
  if (rv == nullptr) {
    throw std::runtime_error("peek_row_replica: rv == nullptr.");
  }
...
template <class StaticConfig>
template <class DataCopier>
bool Transaction<StaticConfig>::write_row_replica(RAH& rah, uint64_t data_size,
                                                  const DataCopier& data_copier) {
  assert(began_);
  if (!rah) return false;
